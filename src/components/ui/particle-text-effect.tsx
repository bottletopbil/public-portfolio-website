...
interface ParticleTextEffectProps {
  ...
  fontFamily?: string
  className?: string
  speedScale?: number // 1 = normal speed; 0.8 = 20% slower
}
...
export function ParticleTextEffect({
    ...
    for (const coordIndex of coordsIndexes) {
      ...
      if (alpha > 0) {
        ...
        if (particleIndex < particles.length) {
          ...
          // Slow down movement and blending by speedScale
          particle.maxSpeed = (Math.random() * 6 + 4) * speedScale
          particle.maxForce = particle.maxSpeed * 0.05
          // Decrease circle particle size by ~50%
          particle.particleSize = Math.random() * 3 + 3
          particle.colorBlendRate = (Math.random() * 0.0275 + 0.0025) * speedScale
  ...
  const animate = () => {
    ...
    const advanceEvery = Math.max(60, Math.round(240 / Math.max(0.01, speedScale)))
    if (frameCountRef.current % advanceEvery === 0) {
  ...
  }, [viewport.width, viewport.height, fontSize, fontFamily, words])

  return (
    <div className={`fixed inset-0 flex items-center justify-center bg-black ${className}`}>
